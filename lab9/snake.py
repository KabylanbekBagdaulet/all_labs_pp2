...
class Food:
    ...
    def show(self):
        pygame.draw.rect(display, food_colour, (food_x, food_y, scale * self.weight, scale * self.weight))
...
def gameLoop():
    ...
    while True:  # Бесконечный цикл игры
        ...
        # Если прошло 5 секунд без еды, меняем её местоположение
        if time.time() - last_eat_time > 5:
            food.new_location()
            last_eat_time = time.time()  # Сбрасываем таймер
